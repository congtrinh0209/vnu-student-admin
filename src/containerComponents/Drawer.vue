  ...
  >
    <template v-slot:prepend>
      <v-list-item two-line :class="mini ? 'px-0 pl-2' : ''">
        ...
        <v-list-item-content>
          <v-list-item-title class="text-drawer mb-2" style="font-size: 16px">{{
    ...
    <v-list dense>
      <v-list-item
        :class="
          menuName === menu.MaMenu
            ? 'item-active mb-3 list-menu'
            : 'mb-3 list-menu'
        "
        v-for="(menu, index) in formatListMenu"
        :key="index"
        @click.stop="goToPage(menu.DuongDan)"
      >
        <v-list-item-icon>
...
export default {
  ...
  created() {
    ...
    // }
    // if (
    //   currentQuery.params.hasOwnProperty("type") &&
    //   currentQuery.params.type == "cho-xu-ly"
    // ) {
    //   vm.menuName = "BaoCaoChoXuLy";
    // }
    // if (
    //   currentQuery.params.hasOwnProperty("type") &&
    //   currentQuery.params.type == "cho-duyet"
    // ) {
    //   vm.menuName = "BaoCaoChoDuyet";
    // }
    // if (
    //   currentQuery.params.hasOwnProperty("type") &&
    //   currentQuery.params.type == "xu-ly-lai"
    // ) {
    //   vm.menuName = "BaoCaoXuLyLai";
    // }
    // if (
    //   currentQuery.params.hasOwnProperty("type") &&
    //   currentQuery.params.type == "bao-cao-moi"
    // ) {
    //   vm.menuName = "BaoCaoMoi";
    // }
    // if (
    //   currentQuery.params.hasOwnProperty("type") &&
    //   currentQuery.params.type == "tra-cuu"
    // ) {
    //   vm.menuName = "TraCuu";
    // }
  ...
  computed: {
    ...
    formatListMenu() {
      ...
      if (vm.userInfo.viTriChucDanh !== "Quản trị hệ thống") {
        ...
        data.forEach((item) => {
          iconMenu.forEach((el) => {
            if (el.path === item.DuongDan)
              dataRender.push({ ...item, iconRender: el.name });
          });
        });
      } else {
        let data = [];
        ...
        vm.listMenu.forEach((item) => {
          iconMenu.forEach((el) => {
            if (el.path === item.DuongDan)
              data.push({ ...item, iconRender: el.name });
          });
        });
        dataRender = data;
      }
