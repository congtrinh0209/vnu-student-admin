    ...
    <v-row no-gutters>
      ...
      <v-col cols="12" sm="1">
        <button
          ...
          @click.stop="getListCategoryNewsFilter"
        >
...
export default {
  ...
  methods: {
    getListCategoryNewsFilter() {
      const vm = this;
      console.log("tim kiếm");
      vm.getLisCategoryNews({ keyword: vm.textSearch });
      vm.textSearch = "";
    },
      ...
      vm.$store
        ...
        .then(function (response) {
          ...
          vm.listCategoryNews = vm.listCategoryNews.reduce(function (res, cur) {
            if (vm.idCategoryNews !== cur.PrimKey) res.push(cur);
            return res;
          }, []);
          vm.total = vm.listCategoryNews.length;
          ...
          if (formData.chuyenMuc.includes(cur.MaDinhDanh)) {
            return [
              ...res,
              {
                ...
                PhanQuyenChuyenMuc: formData.TenNhomQuyen.map((el) => ({
