<template>
  <div>
    <v-row no-gutters>
      ...
      <v-col cols="12" sm="1">
        <button class="btn btn-search primary" @click.stop="getListRightsGroupFilter">
...
export default {
  ...
  methods: {
    getListRightsGroupFilter() {
      const vm = this;
      console.log("tim kiếm");
      vm.getListRightsGroup({ TenNhomQuyen: vm.textSearch});
      vm.textSearch = "";
    },
      ...
      vm.$store
        ...
        .then(function (response) {
          ...
          vm.listRightsGroup = vm.listRightsGroup.reduce(function (res, cur) {
            if (vm.idGroup !== cur.PrimKey) res.push(cur);
            return res;
          }, []);
